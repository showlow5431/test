...
Bracket.Utilities = {
	...
	ConfigsToList = function(FolderName)
		if not isfolder(FolderName) then makefolder(FolderName) end
		if not isfolder(FolderName .. "\\Configs") then makefolder(FolderName .. "\\Configs") end
		if not isfile(FolderName .. "\\Autoload.json") then writefile(FolderName .. "\\Autoload.json", "[]") end

		local AutoLoads = HttpService:JSONDecode(readfile(FolderName .. "\\Autoload.json"))
...
Bracket.Elements = {
	Screen = function()
		...
		ScreenAsset.Name = "Bracket " .. cloneref(game:GetService("HttpService")):GenerateGUID(false)
	...
	Window = function(Window)
		...
		function Window:GetAutoLoadConfig(FolderName)
			if not isfolder(FolderName) then makefolder(FolderName) end
			if not isfile(FolderName .. "\\Autoload.json") then
				writefile(FolderName .. "\\Autoload.json", "[]")
			end

			local AutoLoads = HttpService:JSONDecode(
				readfile(FolderName .. "\\Autoload.json")
			) local AutoLoad = AutoLoads[tostring(game.GameId)]
